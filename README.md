...
- **TeX source drop on a single source**: on `source.php` and `view.php`, drop a TeX-source archive (`.zip`, `.tar.gz` / `.tgz`, `.tar`, or `.tar.bz2` — e.g. arXiv "Download source", which is normally a gzipped tar) to replace the source's `body_text` with the article's TeX-rendered prose. The format is sniffed by magic-byte (so Safari/Finder dropping the extension is fine); zips go through `ZipArchive` and tar variants through `PharData`. The pipeline then picks the root `.tex` file (preferring one with `\documentclass`), inlines `\input{}` / `\include{}` / `\subfile{}` references it can resolve inside the archive, and converts LaTeX to a markdown-ish plain text that the existing viewer can highlight and add notes to (`api/tex.php` + `lib/tex.php`). Body source is recorded as `tex_zip` / `tex_tar` / `tex_tar_gz` / `tex_tar_bz2`. If the source already has anchored notes, the UI confirms before clobbering the text.
  - **Figures**: `\includegraphics` and `figure` environments are resolved against image files inside the archive. Displayable formats (`.png`, `.jpg`, `.gif`, `.svg`, `.webp`, `.avif`) are emitted as inline `![caption](url)` markdown so the viewer renders them as `<img>` next to the prose; non-displayable formats common in TeX bundles (`.pdf`, `.eps`, `.ps`) are emitted as `[Figure: caption](url)` markdown links the reader can click. Captions are preserved as searchable plain-text below each figure. Image segments contribute zero characters to the body's offset model, so highlights and notes on the surrounding prose stay anchored correctly.
  - **Image storage / serving**: staged images live under `data/source-media/<source_id>/<safe-name>` (cleared on each re-upload, and on `delete_source`). They are served via `api/media.php?id=<id>&file=<name>`, a read-only GET endpoint that does `realpath()` containment checks, sets a strict `Content-Security-Policy` (relevant for SVGs), and emits `Cache-Control: private, max-age=86400` + an `ETag` so a long read session doesn't repeatedly fetch the same figure.
...
- **Settings → Database backup & restore**: download a `pg_dump` (SQL or custom), or upload a dump to restore into the configured database (`psql` / `pg_restore --clean --if-exists`). Same auth as other write APIs (`X-Admin-Token`). Restore is destructive.
...
| `EXLIBRIS_PG_DUMP` | No | `pg_dump` | Full path to `pg_dump` if not on the PHP `PATH` (Settings backup). |
| `EXLIBRIS_PSQL` | No | `psql` | Full path to `psql` for plain-SQL restores. |
| `EXLIBRIS_PG_RESTORE` | No | `pg_restore` | Full path to `pg_restore` for custom-format restores. |
| `EXLIBRIS_RESTORE_MAX_BYTES` | No | `536870912` | Max uploaded dump size for restore (bytes). |
...
- `api/backup.php` (POST) uses the same token and returns a **full database dump**; `api/restore.php` (multipart POST) can **overwrite or drop** objects in the configured database from an uploaded file. Anyone with the token can exfiltrate or destroy data. Keep the token secret. The PHP process must allow `proc_open` and file uploads within `upload_max_filesize` / `post_max_size` (some hosts disable these).
